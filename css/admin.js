(function () {
    ...
    const statusMeta = {
        ...
        CHO_BO_SUNG: ['Chờ Admin bổ sung', 'admin-badge--info'], CHO_ADMIN: ['Chờ Admin bổ sung', 'admin-badge--info'],
        DA_GUI: ['Đã gửi bổ sung', 'admin-badge--success'],
        DA_XAC_NHAN: ['Đã xác nhận', 'admin-badge--success'],
        DA_HOAN_TAC: ['Đã xử lý cũ', 'admin-badge--info'], DA_TU_CHOI: ['Đã từ chối', 'admin-badge--danger'], MOI: ['Mới tiếp nhận', ''],
    ...
    let approvalReviewDecision = null;
    let approvalConfirmId = null;
    let paymentProofObjectUrl = null;
    ...
    function formatDateFull(value) {
        if (!value) return '—';
        const parsed = new Date(String(value).replace(' ', 'T'));
        return Number.isNaN(parsed.getTime()) ? String(value) : dateTimeFull.format(parsed);
    }
    ...
    function applyAvatarImage(node, value, fallback) {
        const source = resolveAvatarUrl(value);
        if (!source) { node.textContent = fallback; node.style.backgroundImage = ''; node.classList.remove('has-image'); return; }
        const image = new Image();
        image.onload = () => { node.textContent = ''; node.style.backgroundImage = `url("${source.replace(/"/g, '%22')}")`; node.classList.add('has-image'); };
        image.onerror = () => { node.textContent = fallback; node.style.backgroundImage = ''; node.classList.remove('has-image'); };
        image.src = source;
    }
    ...
    function userAvatar(user = {}) {
        ...
        applyAvatarImage(avatar, user.Avatar || user.avatar, name.charAt(0).toUpperCase());
        return avatar;
    }
    ...
    function loadView(view) {
        ...
        if (view === 'supplemental-evidence') return loadSupplementalEvidence();
    ...
    function renderOrders() {
        ...
        state.orders.forEach((order) => {
            ...
            if(order.PhuongThuc==='BANKING'){const proof=element('div','admin-order-proof');if(order.CoAnhChuyenKhoan){const button=element('button','admin-detail-button','Xem ảnh chuyển khoản');button.type='button';button.addEventListener('click',()=>openPaymentProof(order.MaDH));proof.append(button,element('span','admin-badge admin-badge--success','Đã gửi ảnh'));}else proof.appendChild(element('span','admin-badge','Khách chưa gửi ảnh chuyển khoản'));detailPanel.appendChild(proof);}
    ...
    Object.assign(fieldLabels,{MaDH:'Mã đơn hàng',TenKhachHang:'Khách hàng',TaiKhoanKhachHang:'Tài khoản khách',EmailKhachHang:'Email khách',SoDienThoaiKhachHang:'Số điện thoại khách',SanPhamTrongDon:'Sản phẩm trong đơn',TongTienDonHang:'Tổng tiền đơn hàng',PhuongThucThanhToan:'Phương thức thanh toán',TrangThaiDonHang:'Trạng thái đơn',TrangThaiThanhToan:'Trạng thái thanh toán',ThoiDiemDatHang:'Thời điểm đặt hàng'});
    ...
    function renderSupplementalEvidence(items){
        ...
        if(!items.length){emptyRow(tbody,6,'Chưa có yêu cầu bổ sung minh chứng dành cho tài khoản của bạn.');updateNavBadge($('#navPendingEvidence'),0);return;}
        updateNavBadge($('#navPendingEvidence'),items.filter(item=>item.TrangThai==='CHO_ADMIN').length);
        items.forEach(item=>{
            const row=element('tr');
            row.append(element('td','',formatDate(item.NgayTao)),element('td','','#'+item.MaThayDoi),element('td','',(entityLabels[item.DoiTuong]||item.DoiTuong)+' #'+item.MaDoiTuong));
            const reason=element('td','admin-support-preview');reason.append(element('strong','',item.YeuCau));
            if(item.TraLoi)reason.appendChild(element('span','','Phản hồi gần nhất: '+item.TraLoi));
            row.appendChild(reason);const status=element('td');status.appendChild(badge(item.TrangThai));row.appendChild(status);
            const action=element('td');const actions=element('div','admin-row-actions');
            if(item.TrangThai==='CHO_ADMIN'){const send=element('button','admin-primary','Gửi minh chứng');send.type='button';send.addEventListener('click',()=>openSupplementEvidence(item));actions.appendChild(send);}
            else actions.appendChild(element('span','','Đang chờ Super Admin xét lại'));
            if(Number(item.SoTepDaGui)>0){const view=element('button','admin-detail-button','Xem tệp ('+item.SoTepDaGui+')');view.type='button';view.addEventListener('click',()=>openApprovalEvidence(item.MaThayDoi));actions.appendChild(view);}
            action.appendChild(actions);row.appendChild(action);tbody.appendChild(row);
        });
    }
    let supplementalEvidenceId=null;
    function openSupplementEvidence(item){supplementalEvidenceId=Number(item.MaBoSung);$('#supplementEvidenceTitle').textContent='Bổ sung minh chứng · yêu cầu #'+item.MaThayDoi;$('#supplementEvidenceRequestText').textContent='Super Admin yêu cầu: '+item.YeuCau;$('#supplementEvidenceNote').value='';$('#supplementEvidenceFiles').value='';setStatus($('#supplementEvidenceStatus'));$('#supplementEvidenceDialog').showModal();}
    async function submitSupplementEvidence(event){
        event.preventDefault();const files=[...$('#supplementEvidenceFiles').files];const note=$('#supplementEvidenceNote').value.trim();
        if(note.length<10){setStatus($('#supplementEvidenceStatus'),'Lời nhắn cần ít nhất 10 ký tự.');return;}
        if(!files.length||files.length>5){setStatus($('#supplementEvidenceStatus'),'Chọn từ 1 đến 5 tệp minh chứng.');return;}
        if(files.some(file=>file.size>2*1024*1024)||files.reduce((sum,file)=>sum+file.size,0)>3*1024*1024){setStatus($('#supplementEvidenceStatus'),'Mỗi tệp tối đa 2 MB và tổng dung lượng tối đa 3 MB.');return;}
        const button=$('#supplementEvidenceSubmit');setBusy(button,true,'Đang gửi…');const form=new FormData();form.append('note',note);files.forEach(file=>form.append('files',file));
        try{const response=await fetch(window.API_BASE+'/api/admin/phe-duyet-bo-sung/'+supplementalEvidenceId,{method:'POST',headers:{Authorization:'Bearer '+Auth.getToken()},body:form});const data=await response.json().catch(()=>({}));if(!response.ok||data.success===false)throw new Error(data.message||'Không thể gửi minh chứng.');$('#supplementEvidenceDialog').close();showToast(data.message,'success');loadSupplementalEvidence();}
    ...
    function renderCustomerNotices(items){
        ...
        if(!items.length){emptyRow(tbody,6,'Chưa có thông báo do bạn soạn.');return;}
        items.forEach((item)=>{
            const row=element('tr');
            row.append(element('td','',formatDate(item.NgayTao)),element('td','',item.TieuDe),element('td','',item.MaND?`@${item.TenNguoiNhan||item.MaND}`:'Tất cả khách hàng'),element('td','',`${item.SoDaDoc||0}${item.MaND?'':' lượt đọc'}`));
            const stateCell=element('td');stateCell.appendChild(element('span',`admin-badge ${item.DaThuHoi?'admin-badge--danger':'admin-badge--success'}`,item.DaThuHoi?'Đã thu hồi':'Đã gửi'));row.appendChild(stateCell);
            const action=element('td');
            if(!item.DaThuHoi){const revoke=element('button','danger','Thu hồi');revoke.type='button';revoke.addEventListener('click',async()=>{if(!window.confirm(`Thu hồi thông báo “${item.TieuDe}”?`))return;try{const result=await Auth.request(`/api/admin/thong-bao/${item.MaThongBao}/thu-hoi`,{method:'PATCH'});showToast(result.message,'success');loadCustomerNotices();}catch(error){showToast(error.message,'error');}});action.appendChild(revoke);}else action.textContent='Đã thu hồi';
            row.appendChild(action);tbody.appendChild(row);
        });
    }
    function setupCustomerNoticeComposer(){
        const audience=$('#customerNoticeAudience'),userField=$('#customerNoticeUserField'),search=$('#customerNoticeUserSearch'),results=$('#customerNoticeUserResults');
        audience.addEventListener('change',()=>{userField.hidden=audience.value!=='user';if(audience.value!=='user'){$('#customerNoticeUserId').value='';$('#customerNoticeSelectedUser').textContent='Chưa chọn khách hàng.';results.hidden=true;}});
        let timer=0;search.addEventListener('input',()=>{window.clearTimeout(timer);$('#customerNoticeUserId').value='';$('#customerNoticeSelectedUser').textContent='Chưa chọn khách hàng.';const query=search.value.trim();if(query.length<2){results.hidden=true;return;}timer=window.setTimeout(async()=>{try{const data=await Auth.request(`/api/admin/users?page=1&limit=8&role=user&q=${encodeURIComponent(query)}`);results.innerHTML='';(data.users||[]).forEach((user)=>{const option=element('button','notice-user-option');option.type='button';option.append(element('strong','',user.HoTen||user.TenDangNhap),element('span','',`@${user.TenDangNhap} · ${user.Email||'không có email'} · #${user.MaND}`));option.addEventListener('click',()=>{$('#customerNoticeUserId').value=user.MaND;search.value=`@${user.TenDangNhap}`;$('#customerNoticeSelectedUser').textContent=`Đã chọn ${user.HoTen||user.TenDangNhap} (#${user.MaND}).`;results.hidden=true;});results.appendChild(option);});if(!results.childElementCount)results.appendChild(element('p','admin-empty','Không tìm thấy khách hàng phù hợp.'));results.hidden=false;}catch(error){results.innerHTML='';results.appendChild(element('p','admin-empty',error.message));results.hidden=false;}},220);});
        $('#customerNoticeUploadImage').addEventListener('click',async(event)=>{const file=$('#customerNoticeImageFile').files?.[0];if(!file){$('#customerNoticeImageStatus').textContent='Hãy chọn một ảnh trước.';return;}try{const path=await uploadAdminImage(file,'content',event.currentTarget,$('#customerNoticeImageStatus'));$('#customerNoticeImage').value=path;$('#customerNoticeRemoveImage').hidden=false;$('#customerNoticeImageStatus').textContent='Ảnh đã tải lên và sẽ gửi kèm thông báo.';}catch(error){$('#customerNoticeImageStatus').textContent=error.message;}});
        $('#customerNoticeRemoveImage').addEventListener('click',()=>{$('#customerNoticeImage').value='';$('#customerNoticeImageFile').value='';$('#customerNoticeRemoveImage').hidden=true;$('#customerNoticeImageStatus').textContent='Đã gỡ ảnh khỏi thông báo.';});
        $('#customerNoticeForm').addEventListener('submit',async(event)=>{event.preventDefault();const recipient=audience.value==='all'?'all':$('#customerNoticeUserId').value;if(audience.value==='user'&&!recipient){setStatus($('#customerNoticeStatus'),'Hãy tìm và chọn một khách hàng nhận thông báo.');return;}const button=$('#customerNoticeSubmit');setBusy(button,true,'Đang gửi…');setStatus($('#customerNoticeStatus'));try{const data=await Auth.request('/api/admin/thong-bao',{method:'POST',json:{title:$('#customerNoticeTitle').value.trim(),message:$('#customerNoticeBody').value.trim(),recipient,image:$('#customerNoticeImage').value.trim()}});event.target.reset();$('#customerNoticeUserField').hidden=true;$('#customerNoticeImage').value='';$('#customerNoticeRemoveImage').hidden=true;$('#customerNoticeImageStatus').textContent='JPG, PNG hoặc WebP, tối đa 3 MB.';setStatus($('#customerNoticeStatus'),data.message,true);showToast(data.message,'success');loadCustomerNotices();}catch(error){setStatus($('#customerNoticeStatus'),error.message);}finally{setBusy(button,false);}});
    }
    function changeSummary(item){const after=parsedObject(item.DuLieuSau);const order=item.ThongTinDonHang;if(order)return `Đơn #${order.MaDH} · ${order.TenKhachHang||'Chưa có tên'} · ${order.SanPhamTrongDon||'Chưa có sản phẩm'} · ${formatMoney(order.TongTienDonHang)}`;if(item.DoiTuong==='YeuCauNapTien')return `${item.HanhDong==='APPROVE'?'Duyệt':'Từ chối'} yêu cầu nạp ${formatMoney(after.SoTien||after.amount||0)}`;if(item.HanhDong==='DELETE')return 'Ngừng bán sản phẩm';const preferred=item.DoiTuong==='SanPham'?['TenSP','GiaBan','TonKho','TrangThai']:item.DoiTuong==='NguoiDung'?['HoTen','Email','VaiTro','TrangThai']:item.DoiTuong==='YeuCauHoTro'?['ChuDe','KenhPhanHoi','TrangThai','NoiDung']:[];const keys=[...preferred.filter(key=>after[key]!==undefined),...Object.keys(after).filter(key=>!preferred.includes(key)&&!['NgayCapNhat','NgayTao','NgayXuLy','HinhAnh','AnhChiTiet','Avatar','MoTa','NoiDung'].includes(key))];const summary=keys.slice(0,3).map(key=>{const raw=after[key];const value=String(displayValue(key,raw));return `${fieldLabels[key]||key}: ${value.length>72?`${value.slice(0,69)}…`:value}`;}).join(' · ');return summary||`${entityLabels[item.DoiTuong]||item.DoiTuong}${item.MaDoiTuong?` #${item.MaDoiTuong}`:''} — mở “Xem dữ liệu trước/sau” để xem chi tiết`;}
    function renderApprovals(items){const tbody=$('#approvalRows');tbody.innerHTML='';if(!items.length){emptyRow(tbody,6,'Không có thay đổi phù hợp.');return;}items.forEach(item=>{const row=element('tr');row.append(element('td','',formatDate(item.NgayTao)),element('td','',`@${item.TenDangNhap}`),element('td','',actionLabels[item.HanhDong]||item.HanhDong),element('td','',`${entityLabels[item.DoiTuong]||item.DoiTuong}${item.MaDoiTuong?` #${item.MaDoiTuong}`:''}`));const detail=element('td');detail.appendChild(element('span','admin-detail-summary',changeSummary(item)));if(Number(item.SoLanBoSung)>0){detail.appendChild(element('span','admin-detail-summary',`Admin đã bổ sung ${item.SoLanBoSung} lần${item.NgayBoSung?' · '+formatDate(item.NgayBoSung):''}: ${item.GhiChuBoSung||'đã gửi minh chứng'}`));}const view=element('button','admin-detail-button',item.ThongTinDonHang?'Xem đơn, khách và sản phẩm':'Xem dữ liệu trước/sau');view.type='button';view.addEventListener('click',()=>{const after=item.ThongTinDonHang?{...parsedObject(item.DuLieuSau),...item.ThongTinDonHang}:item.DuLieuSau;openChangeDetail({title:`${actionLabels[item.HanhDong]||item.HanhDong} ${entityLabels[item.DoiTuong]||item.DoiTuong}`,eyebrow:item.ThongTinDonHang?'Chi tiết đơn hàng được xác nhận':'Đối chiếu thay đổi của Admin',meta:[`@${item.TenDangNhap}`,`Thời điểm xử lý: ${formatDate(item.NgayXuLy||item.NgayTao)}`,`Mã thay đổi #${item.MaThayDoi}`,badgeText(item.TrangThai),item.GhiChu?`Lý do: ${item.GhiChu}`:''],before:item.DuLieuTruoc,after,compare:item.ThongTinDonHang?false:Boolean(item.DuLieuTruoc),entity:item.DoiTuong,entityId:item.MaDoiTuong});});detail.appendChild(view);if(item.ThongTinDonHang?.MaDH&&item.CoAnhChuyenKhoan){const proof=element('button','admin-detail-button','Xem ảnh chuyển khoản');proof.type='button';proof.addEventListener('click',()=>openPaymentProof(item.ThongTinDonHang.MaDH));detail.appendChild(proof);}if(Number(item.SoTepBangChung)>0){const files=element('button','admin-detail-button',`Xem bằng chứng (${item.SoTepBangChung})`);files.type='button';files.addEventListener('click',()=>openApprovalEvidence(item.MaThayDoi));detail.appendChild(files);}row.appendChild(detail);const action=element('td');const actions=element('div','admin-row-actions');if(item.TrangThai==='CHO_XEM'){[['XAC_NHAN','Phê duyệt','admin-primary'],['TU_CHOI','Không phê duyệt','danger']].forEach(([decision,label,cls])=>{const button=element('button',cls,label);button.type='button';button.addEventListener('click',()=>decision==='XAC_NHAN'?confirmApproval(item):reviewChange(item.MaThayDoi,decision));actions.appendChild(button);});}else actions.appendChild(badge(item.TrangThai));action.appendChild(actions);row.appendChild(action);tbody.appendChild(row);});}
    function badgeText(status){return statusMeta[status]?.[0]||status||'Không rõ';}
    function confirmApproval(item){approvalConfirmId=Number(item.MaThayDoi);$('#approvalConfirmDescription').textContent=`Bạn có chắc chắn muốn phê duyệt thay đổi #${item.MaThayDoi} (${entityLabels[item.DoiTuong]||item.DoiTuong}${item.MaDoiTuong?' #'+item.MaDoiTuong:''}) do @${item.TenDangNhap} gửi lên không?`;setStatus($('#approvalConfirmStatus'));$('#approvalConfirmDialog').showModal();}
    async function submitApprovalConfirmation(){const button=$('#approvalConfirmSubmit');setBusy(button,true,'Đang phê duyệt…');const form=new FormData();form.append('decision','XAC_NHAN');form.append('note','');form.append('penalty_case','0');form.append('request_evidence','0');try{const response=await fetch(`${window.API_BASE}/api/admin/phe-duyet-thay-doi/${approvalConfirmId}`,{method:'PATCH',headers:{Authorization:`Bearer ${Auth.getToken()}`},body:form});const data=await response.json().catch(()=>({}));if(!response.ok||data.success===false)throw new Error(data.message||'Không thể phê duyệt thay đổi.');$('#approvalConfirmDialog').close();showToast(data.message,'success');loadApprovals();}catch(error){showToast(error.message,'error');}finally{setBusy(button,false);}}
    async function reviewChange(id,decision){approvalReviewId=id;approvalReviewDecision=decision;$('#approvalReviewTitle').textContent='Không phê duyệt thay đổi';$('#approvalReviewDescription').textContent=`Yêu cầu #${id}: ghi lời nhắn giải thích. Có thể gửi ảnh biên bản hoặc tệp kèm theo; nếu bật yêu cầu bổ sung thì Admin phải gửi thêm minh chứng trước khi Super Admin duyệt lại.`;$('#approvalReviewNoteLabel').textContent='Lời nhắn giải thích khi từ chối *';$('#approvalReviewNote').required=true;$('#approvalReviewNote').value='';$('#approvalReviewFiles').value='';$('#approvalRequestEvidence').checked=false;$('#approvalRequestEvidenceField').hidden=false;setStatus($('#approvalReviewStatus'));$('#approvalReviewDialog').showModal();}
    async function submitApprovalReview(event){event.preventDefault();const files=[...$('#approvalReviewFiles').files];const note=$('#approvalReviewNote').value.trim();const requestEvidence=$('#approvalRequestEvidence').checked;if(note.length<10){setStatus($('#approvalReviewStatus'),'Khi từ chối, cần ghi lời nhắn ít nhất 10 ký tự.');return;}if(requestEvidence&&note.length<15){setStatus($('#approvalReviewStatus'),'Hãy mô tả rõ minh chứng cần Admin bổ sung (ít nhất 15 ký tự).');return;}if(files.length>5){setStatus($('#approvalReviewStatus'),'Có thể đính kèm tối đa 5 tệp.');return;}if(files.some(file=>file.size>2*1024*1024)||files.reduce((sum,file)=>sum+file.size,0)>3*1024*1024){setStatus($('#approvalReviewStatus'),'Mỗi tệp tối đa 2 MB và tổng dung lượng tối đa 3 MB.');return;}const button=$('#approvalReviewSubmit');setBusy(button,true,'Đang gửi…');const form=new FormData();form.append('decision','TU_CHOI');form.append('note',note);form.append('penalty_case','0');form.append('request_evidence',requestEvidence?'1':'0');files.forEach(file=>form.append('files',file));try{const response=await fetch(`${window.API_BASE}/api/admin/phe-duyet-thay-doi/${approvalReviewId}`,{method:'PATCH',headers:{Authorization:`Bearer ${Auth.getToken()}`},body:form});const data=await response.json().catch(()=>({}));if(!response.ok||data.success===false)throw new Error(data.message||'Không thể lưu quyết định.');$('#approvalReviewDialog').close();showToast(data.message,'success');loadApprovals();}catch(error){setStatus($('#approvalReviewStatus'),error.message);}finally{setBusy(button,false);}}
    ...
    function updateApprovalKpis(changes, summary = null) {
        ...
        const rejected = Number(summary?.DA_TU_CHOI ?? changes.filter(c => c.TrangThai === 'DA_TU_CHOI').length) || 0;
        animateMetric($('#kpiApprovalReverted'), rejected);
        animateMetric($('#kpiApprovalRollbackable'), confirmed + rejected);
    }
    ...
    function installEvents(){
        ...
        $('#approvalStatus').addEventListener('change',loadApprovals);$('#approvalReviewForm').addEventListener('submit',submitApprovalReview);$('#approvalConfirmSubmit').addEventListener('click',submitApprovalConfirmation);$('#supplementEvidenceRefresh').addEventListener('click',loadSupplementalEvidence);$('#supplementEvidenceForm').addEventListener('submit',submitSupplementEvidence);
        setupCustomerNoticeComposer();$('#customerNoticeRefresh').addEventListener('click',loadCustomerNotices);
        $('#paymentProofDialog').addEventListener('close',()=>{if(paymentProofObjectUrl){URL.revokeObjectURL(paymentProofObjectUrl);paymentProofObjectUrl=null;}$('#paymentProofPreview').removeAttribute('src');});
    ...
    async function init(){
        ...
        applyAvatarImage($('#adminAvatar'), admin.avatar, name.charAt(0).toUpperCase());
