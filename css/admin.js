(function () {
    ...
    async function reviewChange(id,decision){approvalReviewId=id;approvalReviewDecision=decision;$('#approvalReviewTitle').textContent=decision==='XAC_NHAN'?'Phê duyệt thay đổi':'Không phê duyệt thay đổi';$('#approvalReviewDescription').textContent=`Yêu cầu #${id}: giải thích quyết định. Có thể yêu cầu Admin bổ sung minh chứng khi từ chối; tệp chỉ bắt buộc nếu lập biên bản phạt.`;$('#approvalReviewNote').value='';$('#approvalReviewFiles').value='';$('#approvalPenaltyCase').checked=false;$('#approvalRequestEvidence').checked=false;$('#approvalRequestEvidenceField').hidden=decision!=='TU_CHOI';setStatus($('#approvalReviewStatus'));$('#approvalReviewDialog').showModal();}
    async function submitApprovalReview(event){event.preventDefault();const files=[...$('#approvalReviewFiles').files];const note=$('#approvalReviewNote').value.trim();const penalty=$('#approvalPenaltyCase').checked;const requestEvidence=$('#approvalRequestEvidence').checked;if(note.length<10){setStatus($('#approvalReviewStatus'),'Lời giải thích cần ít nhất 10 ký tự.');return;}if(requestEvidence&&note.length<15){setStatus($('#approvalReviewStatus'),'Hãy mô tả rõ minh chứng cần Admin bổ sung (ít nhất 15 ký tự).');return;}if(penalty&&(!files.length||files.length>5)){setStatus($('#approvalReviewStatus'),'Biên bản phạt Admin cần từ 1 đến 5 tệp bằng chứng.');return;}if(files.length>5){setStatus($('#approvalReviewStatus'),'Có thể đính kèm tối đa 5 tệp.');return;}if(files.some(file=>file.size>2*1024*1024)||files.reduce((sum,file)=>sum+file.size,0)>3*1024*1024){setStatus($('#approvalReviewStatus'),'Mỗi tệp tối đa 2 MB và tổng dung lượng tối đa 3 MB.');return;}const button=$('#approvalReviewSubmit');setBusy(button,true,'Đang gửi…');const form=new FormData();form.append('decision',approvalReviewDecision);form.append('note',note);form.append('penalty_case',penalty?'1':'0');form.append('request_evidence',requestEvidence?'1':'0');files.forEach(file=>form.append('files',file));try{const response=await fetch(`${window.API_BASE}/api/admin/phe-duyet-thay-doi/${approvalReviewId}`,{method:'PATCH',headers:{Authorization:`Bearer ${Auth.getToken()}`},body:form});const data=await response.json().catch(()=>({}));if(!response.ok||data.success===false)throw new Error(data.message||'Không thể lưu quyết định.');$('#approvalReviewDialog').close();showToast(data.message,'success');loadApprovals();}catch(error){setStatus($('#approvalReviewStatus'),error.message);}finally{setBusy(button,false);}}
