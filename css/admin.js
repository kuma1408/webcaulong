(function () {
    ...
    async function reviewChange(id,decision){approvalReviewId=id;approvalReviewDecision=decision;$('#approvalReviewTitle').textContent=decision==='XAC_NHAN'?'Phê duyệt thay đổi':'Không phê duyệt thay đổi';$('#approvalReviewDescription').textContent=`Yêu cầu #${id}: thêm lời giải thích và tệp bằng chứng trước khi gửi quyết định.`;$('#approvalReviewNote').value='';$('#approvalReviewFiles').value='';setStatus($('#approvalReviewStatus'));$('#approvalReviewDialog').showModal();}
    async function submitApprovalReview(event){event.preventDefault();const files=[...$('#approvalReviewFiles').files];const note=$('#approvalReviewNote').value.trim();if(note.length<10){setStatus($('#approvalReviewStatus'),'Lời giải thích cần ít nhất 10 ký tự.');return;}if(!files.length||files.length>5){setStatus($('#approvalReviewStatus'),'Chọn từ 1 đến 5 tệp bằng chứng.');return;}if(files.some(file=>file.size>2*1024*1024)||files.reduce((sum,file)=>sum+file.size,0)>3*1024*1024){setStatus($('#approvalReviewStatus'),'Mỗi tệp tối đa 2 MB và tổng dung lượng tối đa 3 MB.');return;}const button=$('#approvalReviewSubmit');setBusy(button,true,'Đang gửi…');const form=new FormData();form.append('decision',approvalReviewDecision);form.append('note',note);files.forEach(file=>form.append('files',file));try{const response=await fetch(`${window.API_BASE}/api/admin/phe-duyet-thay-doi/${approvalReviewId}`,{method:'PATCH',headers:{Authorization:`Bearer ${Auth.getToken()}`},body:form});const data=await response.json().catch(()=>({}));if(!response.ok||data.success===false)throw new Error(data.message||'Không thể lưu quyết định.');$('#approvalReviewDialog').close();showToast(data.message,'success');loadApprovals();}catch(error){setStatus($('#approvalReviewStatus'),error.message);}finally{setBusy(button,false);}}
