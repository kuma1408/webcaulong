(function(){
    ...
    function empty(message='Bạn chưa lưu sản phẩm nào.', heading='0 sản phẩm đã lưu'){
        ...
        grid.appendChild(state); title.textContent=heading;
    }
    ...
    function card(product){
        ...
        const stockValue=Number(product.TonKho); const stockKnown=product.TonKho!==null&&product.TonKho!==undefined&&product.TonKho!==''&&Number.isFinite(stockValue); const inStock=!stockKnown||stockValue>0;
        const stock=document.createElement('span'); stock.className=`wish-stock${inStock?' is-available':' is-unavailable'}`; stock.textContent=stockKnown?(inStock?`Còn ${stockValue} sản phẩm`:'Tạm hết hàng'):'Kiểm tra tồn kho';
        ...
        const cart=document.createElement('button'); cart.type='button'; cart.className='wish-cart'; cart.textContent=inStock?'Thêm giỏ':'Hết hàng'; cart.disabled=!inStock; if(!inStock)cart.setAttribute('aria-label',`${product.TenSP||'Sản phẩm'} hiện đã hết hàng`); else cart.addEventListener('click',()=>addCart(cart,id));
        actions.append(detail,remove,cart); body.append(brand,heading,price,stock,actions); article.append(media,body); return article;
    }
    ...
    async function load(){
        ...
        catch(error){empty(error.message||'Không thể tải danh sách yêu thích. Hãy thử làm mới.','Chưa tải được danh sách')}
    }
